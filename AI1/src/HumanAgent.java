...
public class HumanAgent extends Agents {
    ...
    public Action getNextAction(int deadline,int peopleToSave,int k) { // gets the next move from user and updates current state
        ...
        if(act == -1){ // return 'no-op' action
            ...
            // state updates
            peopleSaved = updateState(deadline,peopleToSave,nextV,k);
            // action
            return new Action(State.getDeadLine(),peopleSaved,nextV,null,0);
        }
    }

    public int updateState(int deadline, int peopleToSave, Vertex nextV,int k){ // returns numOfPeople saved
        int peopleSaved = 0;
